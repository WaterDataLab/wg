...
output: 
  flexdashboard::flex_dashboard:
    orientation: rows
    theme: bootstrap
    # logo: ../etc/logo.jpg # needs to be 50 pixels high
    # favicon: ../etc/wg_logo.png
---
...
vol_in <- adjustOHLC(SPY)
TWTR <- getSymbols("TWTR", from = Sys.Date() - lubridate::years(1), auto.assign = FALSE) 
vol_out <- adjustOHLC(TWTR) * -1
TSLA <- getSymbols("TSLA", from = Sys.Date() - lubridate::years(1), auto.assign = FALSE)
hyd_head <- adjustOHLC(TSLA) * -1
FB <- getSymbols("FB", from = Sys.Date() - lubridate::years(1), auto.assign = FALSE)
pressure <- adjustOHLC(FB)

# value boxes
# icons: https://ionic.io/ionicons/v2
box_vol_in <- valueBox("+201.5 AF", icon = "ion-arrow-down-c", color = pal[6],
                       caption = "Volume injected (last 7 days)")
box_vol_out <- valueBox("-93 AF", icon = "ion-arrow-up-c", color = pal[5],
                       caption = "Volume extracted (last 7 days)")
box_filt_pres <- valueBox("52 PSI", icon = "ion-grid", color = pal[4],
                          caption = "Last recorded filter tank pressure")
box_hyd_head <- valueBox("-134.5 ft (BLS)", icon = "ion-ios-analytics-outline", color = pal[3],
                          caption = "Last recorded hydraulic head")

# accounting page
box_vol_in_ytd <- valueBox("+2,103 AF", icon = "ion-arrow-down-c", color = pal[6],
                          caption = "Volume injected YTD")
box_vol_out_ytd <- valueBox("-5,103 AF", icon = "ion-arrow-up-c", color = pal[5],
                          caption = "Volume extracted YTD")
box_vol_diff_ytd <- valueBox("+1,500 AF", icon = "ion-ios-speedometer-outline", color = pal[12],
