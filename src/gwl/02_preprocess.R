f_preprocess <- function(zzz){
# ith GSA to loop over
# zzz <- unique(gwl$GSA_ID)[20]

# water year type
mwyt <- wyt %>% 
  # TODO: download and join HUC8 boundaries to filter in the next step
  # filter(HUC8 == names(which.max(table(zzz$HUC8)))) %>% 
...
# TODO: abstract into separate functions
...
maoi <- gwl %>% 
  filter(GSA_ID == zzz) %>% 
  group_by(SITE_CODE) %>% 
  mutate(SAMPLE_RANGE = paste(range(MSMT_DATE), collapse = " - "),
         SAMPLE_COUNT = n()) %>% 
  ungroup()


# some sort of SPC on groundwater levels
...
maoi <- maoi %>% 
  ...
  mutate(fall_high   = ifelse(season == "fall",   min(GSE_GWE, na.rm = TRUE), NA),
...
l <- l %>%
  addPolygons(data = st_transform(filter(gsa, GSA_ID == zzz), 4326),
              fillOpacity = 0, 
              color = "red") %>% 
