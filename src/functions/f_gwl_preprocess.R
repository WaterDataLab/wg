f_gwl_preprocess <- function(site_id){
  # ith GSA to loop over
  # site_id <- unique(gwl$GSA_ID)[20]
  
  # water year type
  mwyt <- wyt %>% 
    # TODO: download and join HUC8 boundaries to filter in the next step
    # filter(HUC8 == names(which.max(table(site_id$HUC8)))) %>% 
  ...
  l <- leaflet() %>% 
    ...
    addPolygons(data = st_transform(filter(gsa, GSA_ID == site_id), 4326),
                fillOpacity = 0, 
                color = "red") %>% 
